...
public:
    ...
    void lock()
    {
        ...
        if (res == EOWNERDEAD)
        {
            ...
            throw std::system_error(EOWNERDEAD, std::system_category(), "pthread_mutex_lock failed");
        }
    ...
    bool try_lock()
    {
        ...
        if (res == EOWNERDEAD)
        {
            pthread_mutex_consistent(&m_mutex);
            pthread_mutex_unlock(&m_mutex);
            throw std::system_error(EOWNERDEAD, std::system_category(), "pthread_mutex_trylock failed");
        }
        else if (res == ENOTRECOVERABLE)
    ...
    {
        timespec ts = duration_to_timespec(timeout_duration);
        ...
        if (res == EOWNERDEAD)
        {
            pthread_mutex_unlock(&m_mutex);
            pthread_mutex_consistent(&m_mutex);
            throw std::system_error(EOWNERDEAD, std::system_category(), "pthread_mutex_clocklock failed");
        }
        else if (res == ENOTRECOVERABLE)
    ...
    bool try_lock_until(const std::chrono::time_point<Clock, Duration> &timeout_time)
    {
        ...
        if (res == EOWNERDEAD)
        {
            pthread_mutex_unlock(&m_mutex);
            pthread_mutex_consistent(&m_mutex);
            throw std::system_error(EOWNERDEAD, std::system_category(), "pthread_mutex_timedlock failed");
        }
        else if (res == ENOTRECOVERABLE)
...
private:
    static timespec duration_to_timespec(const std::chrono::nanoseconds &ns)
    {
    ...
    timespec timepoint_to_timespec(std::chrono::system_clock::time_point tp)
    {
